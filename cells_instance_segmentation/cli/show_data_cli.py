...
def _show_images(coco: COCO, images_root: Path, num_of_images: int) -> None:
    ...
    _, axs = plt.subplots(
        nrows=len(image_anns), ncols=2, figsize=(20, 7 * len(image_anns))
    )
...
def show_data(
    config_path: Path = typer.Option(..., help='Path to config with paths'),
    num_of_images: int = typer.Option(default=3, help='Number of images to show'),
) -> None:
